...
// matching note on pages/before-you-book/Payment.jsx) — these are generic
...
export function Payment() {
  const { country, request, price, setPaid } = useVisaGuidanceFlow()
  const copy = VISA_GUIDANCE_FLOW.steps.payment
  const [method, setMethod] = useState('card')

  useEffect(() => {
    document.title = `Secure Payment — ${country.name} | East-West Africa Link`
  }, [country])

  const navigate = useNavigate()

  // No payment processor is integrated anywhere in this project. Submitting
  // just marks the request "paid" locally and moves to the confirmation
  // step, same as every other flow.
  const handleSubmit = (event) => {
    event.preventDefault()
    setPaid(true)
    navigate(`/personal-visa-guidance/${country.slug}/confirmation`)
  }
  ...
  return (
    <>
      ...
      <PlannerStepHero
        ...
        stepper={<PlannerStepper steps={VISA_GUIDANCE_FLOW.stepLabels} current={3} />}
      />
              ...
              <Reveal className="overflow-hidden rounded-2xl border border-border bg-card">
                ...
                <dl className="space-y-2.5 p-5 sm:p-6">
                  <div className="flex justify-between gap-4 text-sm">
                    ...
                    <dd className="font-semibold text-primary">Personal Visa Guidance</dd>
                  </div>
                  <div className="flex justify-between gap-4 text-sm">
                    <dt className="text-muted-foreground">Country</dt>
                    <dd className="text-right font-semibold text-primary">{country.name}</dd>
                  </div>
                  <div className="flex justify-between gap-4 text-sm">
                    <dt className="text-muted-foreground">Price</dt>
                    <dd className="font-semibold text-primary">${price} USD</dd>
                  </div>
