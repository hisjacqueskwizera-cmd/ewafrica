...
const ICONS = { FileText, Search, Clock, Mail, ShieldCheck, Users, Compass }

export function TravelAudit() {
  useEffect(() => {
  ...
  const destinationNames = destinationSlugs
    ...
    .filter(Boolean)
  const price = priceForTravelAudit(destinationSlugs)
  const requestHref = `/travel-planner/travel-audit/request?destinations=${destinationSlugs.join(',')}`
    ...
    <>
      ...
      <div className="border-b border-border">
        <div className="mx-auto flex max-w-7xl items-center gap-1.5 px-4 py-3 text-xs text-muted-foreground sm:px-6 lg:px-8">
          ...
          <ChevronRight className="size-3" aria-hidden="true" />
          <span className="font-semibold text-primary">Travel Audit</span>
        </div>
      </div>
          ...
          <div className="grid gap-10 lg:grid-cols-[1fr_320px]">
            <div>
              ...
              <Reveal delay={125} className="mt-10">
                <h2 className="text-lg font-bold text-primary">{whyItMatters.heading}</h2>
