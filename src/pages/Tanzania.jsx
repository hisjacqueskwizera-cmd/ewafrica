...
// link to /#contact (see the note on TANZANIA_PAGE.services) — the Travel
// Planner card is the one exception, since that flow now actually exists.
...
export function Tanzania() {
  useEffect(() => {
    document.title = 'Explore Tanzania | East-West Africa Link'
  }, [])

  const { hero, services, guide, routes, askUs, trust, farewell } = TANZANIA_PAGE

  return (
    <>
      {/* Hero — the same shared PageIntro used on Home/About/Travel
          Planner (same min-height, right-aligned copy, badge/heading/
          tagline/description grammar, transparent-until-scroll header),
          just rotating through Tanzania's own TZ_HERO_VIDEOS instead of
          the site-wide clip rotation — see the comment on
          TANZANIA_PAGE.hero. */}
      <PageIntro {...hero} backgroundVideos={TZ_HERO_VIDEOS} />

      {/* Travel Services in Tanzania — a dashed-rule-flanked label rather
          than the site's usual SectionTitle, since these four cards are the
          page's real subject and the heading should stay out of the way. */}
                      ...
                      <CardCta
                        to={
                          service.title === 'Travel Planner'
                            ? '/travel-planner?destination=tanzania'
                            : '/#contact'
                        }
                      >
                        {service.cta}
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <Reveal className="text-center">
            ...
            <p className="mx-auto mt-3 max-w-2xl text-sm leading-relaxed text-muted-foreground sm:text-base">
              Travelling beyond Tanzania? We can help you research routes between Tanzania and
