...
export function Gambia() {
  useEffect(() => {
    document.title = 'Explore The Gambia | East-West Africa Link'
  }, [])

  const { hero, services, routes, guide, closing, trust, farewell } = GAMBIA_PAGE

  return (
    <>
      {/* Hero — the same full-viewport hero every destination page shares,
          lifted from About Us. The Gambia's two subheading lines fold into
      ...
      {/* Need Personalized Guidance? — same layout as Senegal's closing
          banner: guidance copy on the left, a photo panel on the right
          listing what we help with. */}
      ...
      {/* Farewell strip — a light, welcoming close, matching Senegal's
          treatment. */}
