import { ChevronRight, Clock, HelpCircle, Info, Mail, MessageCircleQuestion, Search, ShieldCheck } from 'lucide-react'
import { useEffect } from 'react'
import { Link } from 'react-router-dom'
import { CONTACT_INFO, GHANA_LAND_PROPERTY_FLOW, GHANA_LAND_PROPERTY_PAGE } from '../data/siteContent.js'
...
export function LandPropertyGuidance() {
  useEffect(() => {
    document.title = 'Land & Property Guidance | East-West Africa Link'
  }, [])
  ...
  const {
    ...
    stats,
    closing,
  } = GHANA_LAND_PROPERTY_PAGE

  const requestHref = '/ghana/land-property-guidance/request'
    ...
    <>
      ...
      <div className="border-b border-border">
        <div className="mx-auto flex max-w-7xl items-center gap-1.5 px-4 py-3 text-xs text-muted-foreground sm:px-6 lg:px-8">
          ...
          <ChevronRight className="size-3" aria-hidden="true" />
          <span className="font-semibold text-primary">Land & Property Guidance</span>
        </div>
      </div>
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="grid gap-10 lg:grid-cols-[1fr_320px]">
            <div>
              ...
              <Reveal delay={115} className="mt-10">
                <h2 className="text-lg font-bold text-primary">{howWeHelp.heading}</h2>
                <div className="mt-4 max-w-2xl space-y-3 text-sm leading-relaxed text-muted-foreground">
                  {howWeHelp.paragraphs.map((p) => (
                    <p key={p}>{p}</p>
                  ))}
                </div>
              </Reveal>
              ...
              <Reveal delay={130} className="mt-10">
                <h2 className="text-lg font-bold text-primary">{whatWeCover.heading}</h2>
                <ul className="mt-4 grid gap-3 sm:grid-cols-2">
                  {whatWeCover.items.map((item) => (
              ...
              <Reveal delay={165} className="mt-10 rounded-3xl bg-cream p-6 sm:p-8">
                <div className="flex items-center gap-2.5">
                  <MessageCircleQuestion className="size-5 text-copper" aria-hidden="true" />
                  <h2 className="text-lg font-bold text-primary">{examplesOfQuestions.heading}</h2>
                </div>
                <ul className="mt-4 grid gap-2.5 sm:grid-cols-2">
                  {examplesOfQuestions.items.map((item) => (
                    <li key={item} className="flex items-start gap-2.5 text-sm text-muted-foreground">
                      <HelpCircle className="mt-0.5 size-4 shrink-0 text-copper" aria-hidden="true" />
                      {item}
                    </li>
                  ))}
                </ul>
