...
function countryName(slug) {
  return COUNTRIES.find((c) => c.slug === slug)?.name ?? ''
}
...
export function ReviewAnswers() {
  ...
  const { request } = useBorderCrossingFlow()
  const copy = BORDER_CROSSING_FLOW.steps.review
  const editTo = '/travel-planner/border-crossing-guide/request'

  const certainty = BORDER_CROSSING_FLOW.certaintyOptions.find((o) => o.value === request.certainty)
  ...
  return (
    <>
      ...
      <PlannerStepHero
        ...
        stepper={<PlannerStepper steps={BORDER_CROSSING_FLOW.stepLabels} current={2} />}
      />
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <div className="mt-8 grid gap-8 lg:grid-cols-[1fr_320px]">
            <div className="space-y-5">
              ...
              <ReviewSection number={2} title="Travel Route" editTo={editTo}>
                <ReviewRow
                  label="Border Crossings"
                  value={
                    request.crossingCount
                      ? `${request.crossingCount === '5' ? '5 or more' : request.crossingCount} border crossing${request.crossingCount === '1' ? '' : 's'}`
                      : ''
                  }
                />
                <ReviewRow
                  label="Traveling From"
                  value={[countryName(request.fromCountrySlug), request.fromCity].filter(Boolean).join(', ')}
                />
                <ReviewRow
                  label="Traveling To"
                  value={[countryName(request.toCountrySlug), request.toCity].filter(Boolean).join(', ')}
                />
              </ReviewSection>

              <ReviewSection number={3} title="Tentative Travel Plans" editTo={editTo}>
                <ReviewRow
                ...
                />
                <ReviewRow label="How long will you stay?" value={request.duration} />
              </ReviewSection>

              <ReviewSection number={4} title="Purpose of Your Trip" editTo={editTo}>
                <ReviewRow
                  label="Purpose"
              ...
              <ReviewSection number={5} title="Border Crossing Certainty" editTo={editTo}>
                <ReviewRow label="Do you know which crossing to use?" value={certainty?.label} />
                {(request.certainty === 'idea' || request.certainty === 'known') && (
                  <ReviewRow label="Crossing(s) considered" value={request.crossingDetails} />
                )}
              </ReviewSection>

              <ReviewSection number={6} title="Additional Information" editTo={editTo}>
                <ReviewRow label="Your notes" value={request.additionalInfo} />
              </ReviewSection>
              ...
              <div className="flex flex-col-reverse items-center justify-between gap-4 sm:flex-row">
                ...
                <div className="flex flex-col items-center gap-2 sm:items-end">
                  <button
                    type="button"
                    onClick={() => navigate('/travel-planner/border-crossing-guide/payment')}
