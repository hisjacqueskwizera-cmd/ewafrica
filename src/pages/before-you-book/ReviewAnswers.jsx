...
export function ReviewAnswers() {
  ...
  const { request } = useBeforeYouBookFlow()
  const copy = BEFORE_YOU_BOOK_FLOW.steps.review
  const editTo = '/travel-planner/before-you-book-check/request'
  ...
  return (
    <>
      ...
      <PlannerStepHero
        ...
        stepper={<PlannerStepper steps={BEFORE_YOU_BOOK_FLOW.stepLabels} current={2} />}
      />
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <div className="mt-8 grid gap-8 lg:grid-cols-[1fr_320px]">
            <div className="space-y-5">
              ...
              <ReviewSection number={2} title="Planned Destinations" editTo={editTo}>
              ...
              <ReviewSection number={3} title="Tentative Travel Plans" editTo={editTo}>
                ...
                />
                <ReviewRow label="How long do you plan to stay?" value={request.duration} />
              </ReviewSection>

              <ReviewSection number={4} title="What Are You Planning to Book?" editTo={editTo}>
                {BEFORE_YOU_BOOK_FLOW.bookingOptions.map((option) => (
                  <ReviewRow
                    key={option}
                    label={option}
                    value={request.bookingPlans.includes(option) ? 'Yes' : 'No'}
                  />
                ))}
                <ReviewRow
                  label="Other"
                  value={request.bookingPlans.includes('Other') ? request.otherBookingPlan : ''}
                />
              </ReviewSection>

              <ReviewSection number={5} title="Planned Itinerary or Ideas" editTo={editTo}>
                <ReviewRow label="Your notes" value={request.itinerary} />
              </ReviewSection>

              <ReviewSection number={6} title="Specific Questions or Concerns" editTo={editTo}>
                <ReviewRow label="What would you like us to check or advise on?" value={request.questions} />
              </ReviewSection>
              ...
              <div className="flex flex-col-reverse items-center justify-between gap-4 sm:flex-row">
                ...
                <div className="flex flex-col items-center gap-2 sm:items-end">
                  <button
                    type="button"
                    onClick={() => navigate('/travel-planner/before-you-book-check/payment')}
